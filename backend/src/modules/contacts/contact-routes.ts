...
export async function contactRoutes(app: FastifyInstance): Promise<void> {
  ...
  // ── PATCH /api/v1/contacts/:id/ai-auto-reply — toggle AI auto-reply ──────
  app.patch('/api/v1/contacts/:id/ai-auto-reply', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const user = request.user!;
      const { id } = request.params as { id: string };
      ...
      const updated = await prisma.contact.update({
        where: { id },
        data: { metadata: { ...currentMeta, aiAutoReply: enabled } },
        select: { id: true, metadata: true },
      });

      return { id: updated.id, aiAutoReply: enabled };
    } catch (err) {
      logger.error('[contacts] AI auto-reply toggle error:', err);
      return reply.status(500).send({ error: 'Failed to update AI auto-reply setting' });
    }
  });

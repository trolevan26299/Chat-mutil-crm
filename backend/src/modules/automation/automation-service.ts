...
export interface AutomationContext {
  ...
  _depth?: number;
  contact?: { id: string; fullName: string | null; crmName?: string | null; phone: string | null; status: string | null; source?: string | null; assignedUserId?: string | null; metadata?: unknown } | null;
...
function evaluateCondition(condition: AutomationCondition, context: AutomationContext): boolean {
  ...
  // Normalize values for loose comparison (handles string "true"/"false" vs boolean)
  ...
  switch (condition.op) {
    case 'eq': return normalize(current) === normalize(condition.value);
    case 'neq': return normalize(current) !== normalize(condition.value);
...
function getFieldValue(field: string, context: AutomationContext): unknown {
  switch (field) {
    ...
    case 'contact.aiAutoReply': {
      const meta = context.contact?.metadata as Record<string, unknown> | null | undefined;
      return meta?.aiAutoReply ?? false;
    }

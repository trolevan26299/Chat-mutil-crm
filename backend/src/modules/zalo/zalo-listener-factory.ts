...
export function attachZaloListener(ctx: ListenerContext): void {
  ...
  listener.on('message', async (message: any) => {
    try {
      ...
      // Resolve real sticker image URL via zca-js getStickersDetail
      if (contentType === 'sticker') {
        try {
          const stickerId = rawContent?.id ?? rawContent?.catId;
          // Always preserve id/catId in content for URL fallback
          const baseContent = typeof rawContent === 'object' ? rawContent : {};
          let stickerUrl: string | null = null;
          if (stickerId && api.getStickersDetail) {
            const details = await api.getStickersDetail(stickerId);
            const detail = Array.isArray(details) ? details[0] : details;
            if (detail?.stickerWebpUrl || detail?.stickerUrl) {
              stickerUrl = detail.stickerWebpUrl || detail.stickerUrl;
            }
          }
          content = JSON.stringify({
            ...baseContent,
            id: rawContent?.id,
            catId: rawContent?.catId,
            ...(stickerUrl ? { stickerUrl } : {}),
          });
        } catch (e) {
          logger.warn(`[zalo:${accountId}] sticker resolve failed:`, e);
          // Fallback: still store id/catId
